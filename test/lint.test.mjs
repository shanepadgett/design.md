...
function assertDiagnostic(result, rule, severity) {
  assert.ok(
    result.diagnostics.some(
      (diagnostic) =>
        diagnostic.rule === rule && (severity === undefined || diagnostic.severity === severity),
    ),
...
test("document structure diagnostics cover title, duplicate sections, and fence placement", () => {
  assertDiagnostic(
    lint(validDesignMd.replace("# Acme Design", "Acme Design")),
    "missing-title",
    "error",
  );
  ...
  assertDiagnostic(
    lint(
      validDesignMd.replace(
        ...
        "Use {colors.missing} while Acme uses calm surfaces",
      ),
    ),
    "missing-reference",
    "error",
...
test("contrast warnings cover semantic color pairs", () => {
  ...
  assert.ok(
    result.diagnostics.some((diagnostic) => diagnostic.path === "colors.primary/colors.on-primary"),
  );
});
